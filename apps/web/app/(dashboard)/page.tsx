    ...
    <div className="space-y-6">
      ...
      {!isLoading && state.phase !== "error" && (
        <>
          <div className="flex rounded-xl border border-gray-200 bg-gray-50 p-1 gap-1">
            <button
              onClick={() => { setTab("file"); setLinkError(""); }}
              className={["flex flex-1 items-center justify-center gap-2 rounded-lg py-2.5 text-sm font-medium transition-colors", tab === "file" ? "bg-white text-gray-900 shadow-sm border border-gray-200" : "text-gray-500 hover:text-gray-700"].join(" ")}
            >
              <Upload className="h-4 w-4" />
              Upload file
            </button>
            <button
              onClick={() => { setTab("link"); setLinkError(""); }}
              className={["flex flex-1 items-center justify-center gap-2 rounded-lg py-2.5 text-sm font-medium transition-colors", tab === "link" ? "bg-white text-gray-900 shadow-sm border border-gray-200" : "text-gray-500 hover:text-gray-700"].join(" ")}
            >
      ...
      {state.phase === "error" && (
        <>
          ...
          <button
            onClick={() => { reset(); setLinkUrl(""); setLinkError(""); }}
